...
function getInitialFilter(orders) {
  const hasActiveOrders = orders.some(
    (order) =>
      order.status === "PENDING" || order.status === "PROCESSING"
  );

  return hasActiveOrders ? "ACTIVE" : "ALL";
}
...
export default function ProfileOrdersList({ initialOrders }) {
  const [activeFilter, setActiveFilter] = useState(() =>
    getInitialFilter(initialOrders)
  );
  ...
  const filterCounts = useMemo(() => {
    ...
    const processing = initialOrders.filter(
    ...
    const canceled = initialOrders.filter(
      ...
      <div className="border-b border-[#d8b66a]/12">
        <div className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="m-0 w-fit rounded-full border border-[#d8b66a]/14 bg-black/22 px-3 py-1.5 text-xs text-[#f3efe5]/58">
            Показано: {filteredOrders.length} из {initialOrders.length}
          </p>
        </div>
